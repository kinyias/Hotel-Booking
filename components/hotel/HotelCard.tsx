...
const HotelCard = ({hotel}:{hotel: HotelWithRooms}) => {
    ...
    className={cn('col-span-1 cursor-pointer trasition hover:scale-105', isMyHotels && 'cursor-default')}>
            ...
            <div className="flex-1 flex flex-col justify-between h-[210px] gap-1 p-1 py-2 text-sm">
                ...
                <div className="flex justify-between items-center">
                   <div className="flex items-center gap-1">
                    ...
                    <div className="font-semibold text-base">{hotel?.rooms[0].roomPrice} VND</div>
                    <div className="text-xs">/ngày</div>
                    </>}
                   </div>
                   {isMyHotels && <Button onClick={() => router.push(`/hotels/${hotel.id}`)} variant="info">Sửa</Button>}
                </div>
            </div>
        </div>
